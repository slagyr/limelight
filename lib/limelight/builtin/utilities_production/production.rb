...
def load_incompatible_version_scene(production_name, required_version)
  ...
  open_scene("incompatible_version", @incompatible_version_stage, :instance_variables => {:production_name => production_name, :required_version => required_version})
end
...
def load_alert_scene(message)
  ...
  open_scene("alert", @alert_stage, :instance_variables => {:message => message})
end
